...
- **completion.sh** — tab-completion for `gtag`/`chezmoi` (incl. `chezmoi modules`, `chezmoi config set <clé> <valeur><TAB>`) subcommands; hooks git's own completion onto every alias defined in `git-aliases.sh` (`gco sta<TAB>` → `gco staging`, same for `ga`/`gp`/`gd`/`gs`/`glog`/...), the alias → git subcommand table being derived from `git-aliases.sh` itself. Sous **zsh**, ces alias réutilisent la complétion native compsys (`compdef _git gco=git-checkout`) — aucune dépendance à `git-completion.bash` (absent par défaut sur macOS) ; sous **bash**, `git-completion.bash` est cherché aux chemins connus et le hookup est **chargé paresseusement** (stub au démarrage, vrai chargement au premier `<TAB>`), et aucun stub n'est posé s'il est introuvable (la complétion par défaut des alias est préservée). Idem pour go-task et docker (`task`/`docker completion bash`/`zsh`), sautés sous zsh si une complétion native existe déjà ; sous bash, leurs scripts générés dépendent de `_init_completion` (paquet bash-completion) : il est sourcé à la demande aux chemins connus, et à défaut le hookup est abandonné plutôt que d'enregistrer une complétion qui échoue à chaque `<TAB>`. `CHEZMOI_NO_LAZY_COMPLETION=1` revient au chargement eager (tout au démarrage).

...
- **prompt.sh** — powerlevel10k-style PS1 for bash, no nerd fonts, two themes selectable via `chezmoi config set prompt.theme <default|minimal>`: `default` shows time, user@host, cwd (troncature intelligente sur segments entiers, `_PROMPT_PATH_MAXLEN=60`), git branch + dirty marker + ahead/behind, package version (`package.json`/`composer.json`), command duration if >=3s ; `minimal` is a single line, cwd + compact git only. Repère `[ssh]` orange en session distante (les deux thèmes).

...
- feat: `chezmoi config set <clé>` sans valeur liste les choix possibles pour une clé à choix fermé (`prompt.theme` : `default`/`minimal`/`agnoster`, avec le choix actif marqué) au lieu d'échouer avec un simple message d'usage ; inchangé pour une clé à valeur libre (`ssh.modules`)
